...
"""
mapping.launch.py — console-driven SLAM mapping. The teleop stack
(manual.launch.py's nodes) PLUS lidar SLAM in mapping mode, so the operator
drives from the console, watches the map build live in the MAP view, and hits
SAVE MAP when done. This is the agent's "mapping" mode.

    cameras + RPLidar C1 + imu_driver + scan_sectors + safety_gate(assist)
      + motor_controller (motors:=true) + detection/depth BPU (idle)
    + lidar_slam.launch.py  (slam_mode:=mapping, odom_source:=icp):
        icp_odometry (/scan -> odom->base_link) + base_link->laser TF
        + slam_toolbox async mapper (-> /map, map->odom)

The lidar driver is started HERE (not by the SLAM include: run_lidar:=false),
so there is no serial-port collision. Operator teleop is the only /cmd_vel
publisher (the agent enables teleop in mapping mode, same as manual).

    ros2 launch navbot_bringup mapping.launch.py               # dry
    ros2 launch navbot_bringup mapping.launch.py motors:=true  # drives!
...
def generate_launch_description():
    ...
    # Lidar SLAM (mapping). run_lidar:=false — the sllidar_node below owns the
    # serial port; the include only adds icp_odometry + base_link->laser TF +
    # slam_toolbox. Keep odom_source=icp (the verified default).
