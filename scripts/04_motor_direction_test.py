...
Pins (BOARD numbering):
  ENA = 32, ENB = 33   -> PWM6/PWM7, needs dtoverlay=dtoverlay_pwm3 + reboot
...
def force_pwm_enable(pin):
    """Hobot.GPIO quirk: start(0) never writes sysfs 'enable' (it skips the
    write while duty==0) and ChangeDutyCycle() doesn't touch it, so the PWM
    output stays off forever. Write the enable bit ourselves once; it then
    persists across duty changes. Same workaround as motor_controller.py."""
    with open(GPIO.pin_info[pin].pwm_enable, "w") as f:
        f.write("1")


force_pwm_enable(ENA)
force_pwm_enable(ENB)
