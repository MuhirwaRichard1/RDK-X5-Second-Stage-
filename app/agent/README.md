...
Serves the WebSocket API (port 8080) the desktop console connects to, and owns
the `ros2 launch` lifecycle for the drive modes. Protocol + architecture:
...
| `navbot_agent/protocol.py` | message schema v1 + binary video header |
| `navbot_agent/server.py` | websockets server; per-client outbox + latest-wins video slots |
