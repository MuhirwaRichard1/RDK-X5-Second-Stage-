...
Cost estimates are indicative — replace `link/SKU` with your actual supplier before ordering.
"Have" = already on the bench (proven in this repo's earlier work).

| # | Part | Qty | Interface / Voltage | Supplier / SKU | Est. unit | Notes |
|---|---|---|---|---|---|---|
| 1 | RDK X5 (8×A55, ~10 TOPS BPU) | 1 | 5V / 3A | D-Robotics | — (have) | The brain; runs ROS 2 Humble + TROS |
| 2 | HBVCAM OV2710 100° (front) | 1 | USB-2 UVC, 5V | HBVCAM | ~$25 | MJPEG 1280×720; primary nav + detection cam |
| 3 | Wide-angle USB camera (L/R) | 2 | USB-2 UVC, 5V | generic ELP/wide-FOV | ~$18 | Side surround; 640×480 MJPEG is enough |
| 4 | **4-port USB 3.0 hub / extender** | 1 | USB 3.0 (to 1 X5 port) | generic | ~$14 | All 3 cams share **one** X5 USB port via this hub; powered/extender variant for reach (Risk R2) |
...
| 7 | TF-Luna LiDAR (range) | 1 | I2C (3.3–5V) | Benewake | — (have) | Forward safety range; already wired on I2C5 @ 0x10 |
...
| | **Estimated build total (excl. RDK X5 & TF-Luna already owned)** | | | | **≈ $122** | Compute + sensing under target $150 |
...
```
**Rule:** motors and logic share **ground** but not the same regulated 5V — motor current spikes must
not brown out the board (same lesson as the servo note in `RDK_X5_Peripherals.md`).
